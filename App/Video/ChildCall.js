...
export default class ChildCall extends Component {
    
    constructor(props) {
        super(props);
        this.disconectcall=async()=>{
            try {
                const response = await fetch(global.ip+"notification/disconnectstatus?id="+global.cid);
                  const json = await response.json();
                 if(json=="removed")
                 {
                    global.cry=1;
                    this.props.navigation.navigate("MonitorBaby")
                 }
    ...
    componentDidMount() {
        this.interval = setInterval(()=>{this.disconectcall()}, 4000);
      }
    render() {
